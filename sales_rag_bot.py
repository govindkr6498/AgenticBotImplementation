...
class SalesRAGBot:
    def __init__(self, pdf_path: str, model_name: str = "gpt-3.5-turbo-0125"):
        ...
        self.s3_client = boto3.client(
            's3',
            aws_access_key_id='AKIAVLTI4BYUTCDJ3HPP',
            aws_secret_access_key='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',
    ...
    def show_all_records(self):
        """Display all locally stored records."""
        local_path = 'C:/Users/admin/Documents/Document/Bot/src/storeJsonRecord'
        if os.path.exists(local_path):
            try:
                with open(local_path, 'r', encoding='utf-8') as f:
                ...
                logger.error(f"Error displaying records: {e}")
        else:
            print("No local data found. Please fetch data first.")
    ...
    def display_all_aws_data(self):
        """Method to fetch and print all AWS data for verification."""
        data = self.fetch_and_save_aws_data()
        total_records = len(data)
        print(f"\nTotal AWS records fetched: {total_records}")
        if total_records > 0:
            print("Sample record:", json.dumps(data[0], indent=2))
        else:
            print("No data found in AWS.")
    ...
    def _generate_response(self, message: str) -> str:
        try:
            ...
            prompt = f"""You are a helpful sales assistant. Your goals:
            1. Provide helpful info about products/courses
            2. Natural conversation
            3. Stay on topic: {current_topic}
            4. Reference previous context
            5. Expand on topics user asks
            6. Use natural language
            7. Only ask for contact info if genuinely interested
    ...
    def interpret_user_query(self, user_question: str) -> Optional[Dict[str, Any]]:
        ...
        Examples:
        Input: "Show properties with plot number 177"
        Output: {{"field": "Plot_Number", "condition": "equals", "value": "177"}}

        Input: "Show properties with Unit Price 177 and Unit Type Studio Apartment"
        Output: [
            {{"field": "Unit_Price__c", "condition": "equals", "value": "177"}},
            {{"field": "Unit_Type__c", "condition": "equals", "value": "Studio Apartment"}}
        ]
        
        Input: "Find apartments in Dubai under 8000"
        Output: [
            {{"field": "District", "condition": "contains", "value": "Dubai"}},
            {{"field": "Unit_Price__c", "condition": "less_than", "value": "8000"}}
        ]
    ...
    def filter_data_by_criteria(self, data: List[Any], criteria: Union[Dict, List[Dict]]) -> List[Any]:
        if not criteria:
            return data
    ...
    def fetch_and_filter_data(self, user_question: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not filtered:
                return [{"message": "No records match your criteria"}]
    ...
    def process_user_query(self, message: str) -> Optional[Dict[str, Any]]:
        command_keywords = ["show", "find", "search", "filter", "get", "list"]
        if any(keyword in message.lower() for keyword in command_keywords):
            ...
            # Format as table
            # Inside your `process_user_query()` method
            if not table_data:
...
def main():
    ...
    try:
        ...
        pdf_path = '/home/ubuntu/AgenticBotImplementation/Emaar_FAQ.pdf'
